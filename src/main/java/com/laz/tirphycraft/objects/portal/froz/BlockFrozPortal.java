package com.laz.tirphycraft.objects.portal.froz;
...
public class BlockFrozPortal extends Block implements IMetaName {
	...
	public BlockFrozPortal() {
		super(Material.PORTAL);
		setUnlocalizedName("froz_portal");
		setRegistryName("froz_portal");
	...
	public void onBlockAdded(World worldIn, BlockPos pos, IBlockState state) {
		if (state == BlockInit.FROZ_PORTAL.getDefaultState()) {
			for (int i = -1; i < 2; i++) {
				for (int j = -1; j < 2; j++) {
					if (!(i == 0 && j == 0))
						worldIn.setBlockState(pos.add(i, j, 0), BlockInit.FROZ_PORTALNULL.getDefaultState());
			...
			for (int i = -1; i < 2; i++) {
				for (int j = -1; j < 2; j++) {
					if (!(i == 0 && j == 0))
						worldIn.setBlockState(pos.add(0, j, i), BlockInit.FROZ_PORTALNULL.getDefaultState());
	...
	public void onEntityCollidedWithBlock(World worldIn, BlockPos pos, IBlockState state, Entity entityIn) {
		...
		if (entityIn instanceof EntityPlayerMP && entityIn.dimension != ConfigHandler.DIMENSION_FROZ_ID) {
			MinecraftServer minecraftserver = FMLCommonHandler.instance().getMinecraftServerInstance();
			if (minecraftserver != null) {
				minecraftserver.getCommandManager().executeCommand((EntityPlayerMP) entityIn, "TP " + ConfigHandler.DIMENSION_FROZ_ID);
	...
	public AxisAlignedBB getBoundingBox(IBlockState state, IBlockAccess source, BlockPos pos) {
		if (blockState != BlockInit.FROZ_PORTAL.getDefaultState())
