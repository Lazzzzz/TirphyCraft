...
public class BlockCreseolRune4 extends Block implements IHasModel {

	private Random rand = new Random();

	public BlockCreseolRune4() {
		super(Material.WOOD);
		setUnlocalizedName("creseol_rune4");
		setRegistryName("creseol_rune4");
		...
		if (!list.isEmpty()) {
			...
			for (int i = 0; i <= list.size() - 1; i++) {
				((EntityLivingBase) list.get(i))
						.addPotionEffect(new PotionEffect(MobEffects.WEAKNESS, 100, 0, false, false));
			}
