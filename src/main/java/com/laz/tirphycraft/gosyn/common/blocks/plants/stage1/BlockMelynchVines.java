...
public class BlockMelynchVines extends Block implements net.minecraftforge.common.IShearable, IHasModel
{
    ...
    public BlockMelynchVines()
    {
        super(Material.VINE);
		setUnlocalizedName("melynch_vines");
		setRegistryName("melynch_vines");
            ...
            if (worldIn.rand.nextInt(4) == 0 && worldIn.isAreaLoaded(pos, 4)) // Forge: check area to prevent loading unloaded chunks
            {
                ...
                else
                {
                    if (pos.getY() > 1)
                    {
                        ...
                        if (block.getMaterial(this.getDefaultState()) == Material.AIR)
                        {
