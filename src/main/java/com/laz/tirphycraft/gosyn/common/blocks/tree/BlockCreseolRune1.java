...
public class BlockCreseolRune1 extends Block implements IHasModel {

	private Random rand = new Random();

	public BlockCreseolRune1() {
		super(Material.WOOD);
		setUnlocalizedName("creseol_rune1");
		setRegistryName("creseol_rune1");
			...
			if (playerIn.inventory.getCurrentItem().getItem() == ItemInit.CRESEOL_TABLET_EMPTY
				...
				if (rand.nextInt(5) == 0) {
					...
					worldIn.setBlockState(pos,
							BlockInit.CRESEOL_BARK.getDefaultState().withProperty(BlockLog.LOG_AXIS, EnumAxis.Y));
		...
		if (!list.isEmpty()) {
			...
			for (int i = 0; i <= list.size() - 1; i++) {
				((EntityLivingBase) list.get(i))
						.addPotionEffect(new PotionEffect(MobEffects.RESISTANCE, 100, 0, false, false));
			}
