...
public class GosynTemplate implements IChunkGenerator {

	protected static final IBlockState STONE = BlockInit.ROCK.getDefaultState();
	protected static final IBlockState GRAVEL = BlockInit.ROCK.getDefaultState();
	...
	public GosynTemplate(World worldIn, long seed) {
			...
			for (int x = 0; x < 5; x++) {
				...
				for (int bx = -1; bx <= 1; bx++) {
					for (int bz = -1; bz <= 1; bz++) {
						...
						thicks[x + z * 5]++;
						/*
						 * if (biome == BiomeInit.FROZ_SLAWOMIR) { thicks[x + z * 5]++; }
						 */
			...
			for (int x = 0; x < 16; x++) {
				...
				if (generateForest) {
					...
					for (int y = 127; y >= 0; y--) {
						...
						if (currentBlock == BlockInit.ROCK) {
							topLevel = y;
							break;
						}
					}
