...
public class WorldGenNoxisLakes extends WorldGenerator
{
    private final Block block;

    public WorldGenNoxisLakes(Block blockIn)
    {
        this.block = blockIn;
    }
                ...
                for (int j2 = 0; j2 < 16; ++j2)
                {
                    for (int k3 = 0; k3 < 16; ++k3)
                    {
                        for (int k4 = 0; k4 < 8; ++k4)
                        {
                            ...
                            {
                            	if (rand.nextBoolean()) worldIn.setBlockState(position.add(j2, k4, k3), BlockInit.NOXIS_ASH_LIT.getDefaultState(), 2);
                            	else worldIn.setBlockState(position.add(j2, k4, k3), BlockInit.NOXIS_STONE.getDefaultState(), 2);
                            }
                        }
                    }
                }
