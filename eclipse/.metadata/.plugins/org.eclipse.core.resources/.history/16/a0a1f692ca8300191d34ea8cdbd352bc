...
public class WorldGenPortalLaputa extends WorldGenerator implements IStructure {
	public String structureName;

	public WorldGenPortalLaputa(String name) {
	...
	private boolean checkForSawn(Template template, World world, BlockPos pos) {
		...
		for (int i = 0; i < template.getSize().getX(); i++) {
			for (int j = 0; j < template.getSize().getZ(); j++) {
				...
				if (!b.equals(BlockInit.NOXIS_GRASS)) {
					flag = false;
				}
