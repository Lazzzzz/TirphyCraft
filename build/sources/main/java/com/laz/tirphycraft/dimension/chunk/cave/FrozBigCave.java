...
public class FrozBigCave extends MapGenBaseMeta
{
    @Override
    ...
    protected void recursiveGenerate(World par1World, int xChunkCoord, int zChunkCoord, int origXChunkCoord, int origZChunkCoord, ChunkPrimer primer)
    {
        if (this.rand.nextInt(400) == 0)
        {
            ...
            this.generateLargeCaveNode(this.rand.nextLong(), origXChunkCoord, origZChunkCoord, primer, xPos, yPos, zPos, par1World);
        }
    }

    protected void generateLargeCaveNode(long par1, int origXChunkCoord, int origZChunkCoord, ChunkPrimer primer, double xPos, double yPos, double zPos, World par1World)
    {
        this.generateCaveNode(par1, origXChunkCoord, origZChunkCoord, primer, xPos, yPos, zPos, 1.0F + this.rand.nextFloat() * 6.0F, 10.0F, 10.0F, -1, -1, 0.2D, par1World);
    }

	protected void generateCaveNode(long par1, int origXChunkCoord, int origZChunkCoord, ChunkPrimer primer,
			double xPos, double yPos, double zPos, float par12, float par13, float par14, int par15, int par16,
			double heightMultiplier, World par1World)
    {
