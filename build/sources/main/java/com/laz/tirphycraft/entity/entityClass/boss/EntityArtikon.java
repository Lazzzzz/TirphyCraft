...
public class EntityArtikon extends EntityMob {
	...
	public void onLivingUpdate() {
		...
		List list = this.world.getEntitiesWithinAABB(EntitySkull.class, box);
		if (!list.isEmpty()) {
	...
	public boolean hitByEntity(Entity entityIn) {
		...
		int r = random.nextInt(((int) this.getHealth() / 4) + 1);
		if (r == 1) {
			this.spawning = true;
		}
			...
			if (this.spawning) {
				...
				if (this.timer == 0) {
					...
					if (this.mobCounter > 0) {
						...
						EntitySkull m = new EntitySkull(this.world);
						m.setPosition(this.posX, this.posY, this.posZ);
						m.motionX = view.x * ra.nextFloat() - 0.5;
						m.motionY = view.y * ra.nextFloat();
						m.motionZ = view.z * ra.nextFloat() - 0.5;
