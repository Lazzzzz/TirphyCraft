...
public class BlockCreseolRune2 extends Block implements IHasModel{

	private Random rand = new Random();
	
	public BlockCreseolRune2() {
		super(Material.WOOD);
		setUnlocalizedName("creseol_rune2");
		setRegistryName("creseol_rune2");
		...
		if (!list.isEmpty()) {
			...
			for (int i = 0; i <= list.size() - 1; i++) {
				((EntityLivingBase) list.get(i))
						.addPotionEffect(new PotionEffect(MobEffects.SPEED, 100, 0, false, false));
			}
