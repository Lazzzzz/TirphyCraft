...
public class spawnerUtil {
	public void placeSpawner(World world, BlockPos pos, int id, Template template) {
				...
				for (int z = 0; z <= template.getSize().getZ(); z++) {
					...
					if (world.getTileEntity(tmp) != null) {
						...
						if (world.getTileEntity(tmp) instanceof TileEntityMobSpawner) {
							...
							spawner.getSpawnerBaseLogic().setEntityId(EntityList.getKey(EntityList.getClassFromID(id)));
						}
					}
				}
			}
