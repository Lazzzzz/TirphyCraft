...
public class WorldGenGeantTreeFroz extends WorldGenHugeTrees {
	private static final IBlockState TRUNK = BlockInit.LOG_FROZ.getDefaultState();
	private static final IBlockState LEAF = BlockInit.LEAVES_FROZ.getDefaultState()
			.withProperty(BlockLeaves.CHECK_DECAY, Boolean.valueOf(false));
	private static final IBlockState PODZOL = Blocks.GRASS.getDefaultState();
	private final boolean useBaseHeight;

	public WorldGenGeantTreeFroz() {
	...
	public boolean generate(World worldIn, Random rand, BlockPos position) {
		if (worldIn
				.getBlockState(new BlockPos(position.getX() + 1, position.getY() - 1, position.getZ())) == Blocks.GRASS
						.getDefaultState()
				&& worldIn.getBlockState(
						new BlockPos(position.getX(), position.getY() - 1, position.getZ() + 1)) == Blocks.GRASS
								.getDefaultState()
				&& worldIn.getBlockState(
						new BlockPos(position.getX() + 1, position.getY() - 1, position.getZ() + 1)) == Blocks.GRASS
								.getDefaultState()) {
