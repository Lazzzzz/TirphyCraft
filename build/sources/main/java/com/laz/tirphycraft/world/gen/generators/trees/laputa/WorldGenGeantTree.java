...
public class WorldGenGeantTree extends WorldGenHugeTrees {
	private static final IBlockState TRUNK = BlockInit.LOG_MYSTICAL.getDefaultState();
	private static final IBlockState LEAF = BlockInit.LEAVES_YELLOW.getDefaultState().withProperty(BlockLeaves.CHECK_DECAY, Boolean.valueOf(false));
	...
	public WorldGenGeantTree() {
		super(false, 11, 5, TRUNK, LEAF);
		this.useBaseHeight = true;
	}

	public boolean generate(World worldIn, Random rand, BlockPos position) {
		if (worldIn
				.getBlockState(new BlockPos(position.getX() + 1, position.getY() - 1,
						position.getZ())) == BlockInit.LAPUTA_GRASS.getDefaultState()
				&& worldIn.getBlockState(new BlockPos(position.getX(), position.getY() - 1,
						position.getZ() + 1)) == BlockInit.LAPUTA_GRASS.getDefaultState()
				&& worldIn.getBlockState(new BlockPos(position.getX() + 1, position.getY() - 1,
						position.getZ() + 1)) == BlockInit.LAPUTA_GRASS.getDefaultState()) {
	...
	private void placePodzolAt(World worldIn, BlockPos pos) {
		for (int i = 2; i >= -3; --i) {
			...
			if (block.canSustainPlant(iblockstate, worldIn, blockpos, net.minecraft.util.EnumFacing.UP,
