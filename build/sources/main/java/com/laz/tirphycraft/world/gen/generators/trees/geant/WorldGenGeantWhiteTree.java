...
public class WorldGenGeantWhiteTree extends WorldGenHugeTrees {
	private static final IBlockState TRUNK = Blocks.LOG.getDefaultState();
	private static final IBlockState LEAF = BlockInit.LEAVES_WHITE.getDefaultState()
			.withProperty(BlockLeaves.CHECK_DECAY, Boolean.valueOf(false));
	private static final IBlockState PODZOL = BlockInit.NOXIS_GRASS.getDefaultState();
	private final boolean useBaseHeight;

	public WorldGenGeantWhiteTree() {
	...
	public boolean generate(World worldIn, Random rand, BlockPos position) {
		if (worldIn
				.getBlockState(new BlockPos(position.getX() + 1, position.getY() - 1,
						position.getZ())) == BlockInit.NOXIS_GRASS.getDefaultState()
				&& worldIn.getBlockState(new BlockPos(position.getX(), position.getY() - 1,
						position.getZ() + 1)) == BlockInit.NOXIS_GRASS.getDefaultState()
